...
class JobListing(BaseModel):
    ...
    expires_at: Optional[datetime] = Field(
        None, description="When the job posting expires"
    )
    ...
    experience_level: Optional[str] = Field(
        None, description="Entry, Mid, Senior, etc."
    )

    # Metadata
    raw_data: Optional[Dict[str, Any]] = Field(
        default_factory=dict, description="Original ATS data"
    )
...
class ATSConnector(ABC):
    ...
    async def fetch_all_jobs(
        self, batch_size: int = 50, max_jobs: Optional[int] = None, **filters
    ) -> List[JobListing]:
        """
        ...
        while True:
            ...
            # Fetch batch
            batch = await self.fetch_jobs(limit=batch_limit, offset=offset, **filters)
    ...
    async def fetch_new_jobs(
        self, since: Optional[datetime] = None, **filters
    ) -> List[JobListing]:
        """
