...
class SecureAPIClient {
  ...
  async getScores(resumeId: string, limit: number = 100) {
    try {
      ...
      if (errorMsg.includes('not found') || 
          ...
          errorMsg.includes('resource was not found')) {
        return []
      }
      throw error
    }
  ...
  async exportScores(resumeId: string, format: 'csv' | 'json' = 'csv') {
    // Use the central request method for consistency
    // Note: The request method handles different content types including blobs
    return this.request<Blob>(
      `/scores/export?resume_id=${resumeId}&format=${format}&include_details=true`,
      {
        method: 'POST',
      }
    )
