...
class ResumeProcessor:
    ...
    def _load_skills_vocabulary(self) -> Dict[str, Dict[str, Any]]:
        ...
        if not os.path.exists(csv_path):
            logger.warning(f"Skills vocabulary not found at {csv_path}, using empty vocabulary")
            return vocab
    ...
    async def _extract_skills_embeddings(
        ...
        try:
            ...
            remaining_skills = [
                skill for skill in self.skills_vocab.values()
    ...
    async def _extract_skills_openai(
        ...
        try:
            ...
            prompt = prompt_template.format(
                ...
                existing_skills=', '.join(existing_skills.keys()) if existing_skills else 'None',
                vocabulary_sample=', '.join(vocab_list[:50])  # Show sample of vocabulary
            )
            
            response = await self.openai_client.chat.completions.create(
                model="gpt-4o-mini",
                messages=[
                    {"role": "system", "content": "You are a technical recruiter expert at identifying skills in resumes. Only extract skills that are explicitly mentioned or strongly implied in the text."},
    ...
    def _load_prompt_template(self) -> str:
        ...
        prompt_path = os.path.join(settings.CONFIG_DIR, "prompts", "skill_extraction.txt")
