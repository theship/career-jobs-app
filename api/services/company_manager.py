...
class CompanyManager:
    ...
    async def get_all_companies(
        self, active_only: bool = True, ats_system: Optional[str] = None
    ) -> List[Dict]:
        """
    ...
    async def get_companies_for_ingestion(
        self, limit: int = 50, ats_system: Optional[str] = None
    ) -> List[Dict]:
        """
        ...
        try:
            ...
            query = (
                self.supabase.table("target_companies").select("*").eq("active", True)
            )
            ...
            result = (
                query.order("priority", desc=False)
                .order("last_successful_fetch", desc=False, nullsfirst=True)
                .limit(limit)
                .execute()
            )
            ...
            for company in companies:
                ...
                last_fetch = datetime.fromisoformat(
                    company["last_successful_fetch"].replace("Z", "+00:00")
                )
    ...
    async def update_company(self, company_uuid: str, **updates) -> Optional[Dict]:
        """
        ...
        try:
            ...
            result = (
                self.supabase.table("target_companies")
                .update(updates)
                .eq("id", company_uuid)
                .execute()
            )
    ...
    async def record_fetch_attempt(
        ...
        try:
            ...
            if success:
                updates["last_successful_fetch"] = datetime.now(
                    timezone.utc
                ).isoformat()
            ...
            self.supabase.table("target_companies").update(updates).eq(
                "id", company_uuid
            ).execute()
    ...
    async def update_ingestion_history(self, history_id: str, **updates) -> None:
        """
        ...
        """
        try:
            self.supabase.table("ingestion_history").update(updates).eq(
                "id", history_id
            ).execute()
        except Exception as e:
    ...
    async def get_company_stats(
        self, company_uuid: Optional[str] = None, days: int = 30
    ) -> Dict:
        """
        ...
        try:
            ...
            query = (
                self.supabase.table("ingestion_history")
                .select("*")
            ...
            stats = {
                ...
                "total_embeddings": sum(
                    h.get("embeddings_generated", 0) for h in history
                ),
                "avg_duration_ms": 0,
                "success_rate": 0,
            }
    ...
    async def delete_company(self, company_uuid: str) -> bool:
        ...
        """
        try:
            self.supabase.table("target_companies").delete().eq(
                "id", company_uuid
            ).execute()
            return True
        except Exception as e:
