...
async def upload_resume(
    ...
    try:
        ...
        file_path = await storage_service.upload_resume(
            file=file, user_id=current_user["sub"]
        )
...
async def list_resumes(
    current_user: dict = Depends(get_current_user), db=Depends(get_db_connection)
):
...
async def delete_resume(
    ...
    async with db.pool.acquire() as conn:
        ...
        await conn.execute(
            "DELETE FROM resume_versions WHERE resume_id = $1", resume_id
        )
