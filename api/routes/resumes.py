...
async def upload_resume(
    ...
    if not file.filename.endswith((".pdf", ".docx", ".txt")):
        await activity_logger.log_action_complete(
            log_id=log_id,
            success=False,
            error_details="Invalid file type. Only PDF, DOCX, TXT supported.",
        )
        raise HTTPException(
    ...
    try:
        ...
        has_token = bool(current_user.get("token"))
        trusted = current_user.get("trusted_service")
        logger.info(
            f"Current user info: user_id={current_user.get('user_id')}, "
            f"has_token={has_token}, trusted_service={trusted}"
        )
        ...
        try:
            ...
            if "row-level security policy" in str(storage_error):
                logger.error(
                    "RLS policy violation - check Supabase settings"
                )
            raise
        ...
        logger.info(
            f"Text extracted: {len(text_content)} chars in {text_extraction_time}ms"
        )
        ...
        logger.info(
            f"Skills extracted: {len(skills_data.skills)} in {skills_extraction_time}ms"
        )
        ...
        try:
            ...
            logger.info(
                f"Database insert successful: "
                f"resume_id = {insert_response.data[0]['resume_id'] if insert_response.data else 'unknown'}"
            )
        ...
        logger.info(
            f"Resume upload complete: resume_id={resume_record['resume_id']}, "
