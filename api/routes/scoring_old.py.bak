...
async def get_resume_data(resume_id: str, supabase):
    ...
    try:
        ...
        if embedding is None:
            raise HTTPException(
                status_code=500, detail="Failed to parse resume embedding"
            )
        ...
        for job in jobs:
            if job.get("embedding"):
                ...
                if parsed_embedding is not None:
                    ...
                    logger.warning(
                        f"Failed to parse embedding for job {job.get('job_id')}"
                    )
...
async def get_scores(
    ...
    if current_user.get("token") and current_user.get("token") != "test":
        ...
        try:
            ...
            logger.warning(
                f"Failed to use authenticated client: {e}, falling back to service client"
            )
            from api.utils.database import get_supabase_service_client
        ...
        supabase = get_supabase_service_client()
        logger.info(
            f"Using service client for user {current_user['user_id']} (trusted service or test)"
        )
    ...
    try:
        ...
        if not response.data:
            logger.info(
                f"No scores found for resume {resume_id}, user {current_user['user_id']}"
            )
            return []
...
async def run_scoring(
    ...
    try:
        ...
        logger.info(
            f"Fetched {len(jobs_data)} jobs with embeddings in {jobs_fetch_time}ms for resume {request.resume_id}"
        )
        await activity_logger.update_action_progress(
            log_id=log_id,
            status="in_progress",
        ...
        logger.info(
            f"Ranking completed for resume {request.resume_id}: {len(scores)} scores generated from {len(jobs_data)} jobs"
        )
        ...
        logger.info(
            f"About to store {len(scores)} scores for resume {request.resume_id}"
        )
        try:
            ...
            logger.error(
                f"Failed to store scores for resume {request.resume_id}: {e}",
                exc_info=True,
            )
...
async def export_scores(
    ...
    try:
        ...
        if not response.data:
            ...
            logger.info(
                f"No stored scores found for resume {resume_id}, running scoring on-the-fly"
            )
            ...
            await activity_logger.update_action_progress(
                ...
                status="in_progress",
                progress_data={
                    "stage": "running_scoring",
                    "message": "Generating scores for export...",
                },
            )
            ...
            if not jobs_data:
                ...
                raise HTTPException(
                    status_code=404, detail="No jobs available for scoring"
                )
            ...
            for score in scores:
                job_info = next(
                    (j for j in jobs_data if j["job_id"] == score.job_id), {}
                )
...
def store_scoring_results(
    ...
    try:
        ...
        supabase.table("scores").delete().eq("resume_id", resume_id).eq(
            "user_id", user_id
        ).execute()
