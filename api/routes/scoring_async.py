...
class ScoringRequest(BaseModel):
    ...
    batch_size: int = Field(10, ge=1, le=50, description="Batch size for progressive updates")
...
async def process_scoring_async(
    ...
    try:
        ...
        redis.setex(task_key, 600, json.dumps({
            "task_id": task_id,
            "resume_id": resume_id,
            "user_id": user_id,
        ...
        # Fetch resume data
        resume_response = supabase.table("resumes").select("*").eq("resume_id", resume_id).single().execute()
        ...
        if isinstance(resume_embedding, str):
            try:
                ...
                if resume_embedding.startswith("np."):
                    ...
                    cleaned = resume_embedding.replace("np.str_('", "").replace("')", "").replace("np.float32(", "").replace(")", "")
        ...
        logger.info(f"Task {task_id}: Processing {total_jobs} jobs for resume {resume_id}")
        ...
        for i in range(0, total_jobs, batch_size):
                ...
                if isinstance(job_embedding, str):
                    try:
                        ...
                        if job_embedding.startswith("np."):
                            # Extract the array from numpy string representation
                            # Remove numpy prefixes and parse as literal
                            cleaned = job_embedding.replace("np.str_('", "").replace("')", "").replace("np.float32(", "").replace(")", "")
            ...
            logger.info(f"Task {task_id}: Processed batch {batch_num}, {processed}/{total_jobs} jobs")
        ...
        logger.info(f"Task {task_id}: Completed scoring, found {len(final_scores)} matches")
