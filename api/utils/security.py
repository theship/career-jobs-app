...
def validate_csv(file_content: bytes, filename: str) -> Tuple[pd.DataFrame, str]:
    ...
    if len(file_content) > MAX_CSV_SIZE:
        raise FileSecurityError(
            f"CSV file too large. Maximum size is {MAX_CSV_SIZE // 1024 // 1024}MB"
        )
    ...
    try:
        ...
        try:
            ...
            raise FileSecurityError(
                "Could not decode CSV file. Please ensure it's a valid text file"
            )
    ...
    try:
        ...
        for col in df.columns:
            df[col] = df[col].apply(
                lambda x: sanitize_text(str(x)) if pd.notna(x) else ""
            )
