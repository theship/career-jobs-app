    ...
    for company in additional_companies:
        ...
        try:
            result = await company_manager.add_company(
                ...
                priority=(
                    1
                    if company["industry"] in ["AI/ML", "Fintech", "Developer Tools"]
                    else 2
                ),
                check_frequency_days=1,
                metadata={},
...
"""
    ...
    for company in final_companies[:10]:
        logger.info(
            f"  - {company['ats_system']}: {company['display_name']} (priority={company['priority']})"
        )
