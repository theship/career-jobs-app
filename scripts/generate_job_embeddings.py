...
async def main():
    ...
    if not all([supabase_url, supabase_key, openai_api_key]):
        ...
        logger.error(
            "Required: SUPABASE_URL, SUPABASE_ANON_KEY/SERVICE_KEY, OPENAI_API_KEY"
        )
        return
    ...
    all_jobs_response = (
        supabase.table("job_postings")
        .select("job_id, title, description_text, requirements_text, embedding")
        .execute()
    )

    jobs_without_embeddings = [
        job for job in all_jobs_response.data if not job.get("embedding")
    ]
