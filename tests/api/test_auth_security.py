...
from api.services.auth import get_current_user, verify_service_secret
...
class TestServiceSecretAuthentication:
    ...
    def test_get_current_user_with_valid_headers(self):
        ...
        # This simulates what Next.js would send
        header = (
        ...
        payload = (
            base64.urlsafe_b64encode(
                json.dumps({"sub": "user-123", "email": "test@example.com"}).encode()
            )
            .decode()
            .rstrip("=")
        )
        ...
        user = get_current_user(
            ...
            x_user_token=mock_jwt_token,
        )

        assert user["user_id"] == "user-123"
        assert user["email"] == "test@example.com"
        assert user["token"] == mock_jwt_token
...
class TestSecurityHeaders:
    ...
    def test_api_requires_service_authentication(self):
        ...
        response = client.post("/api/v1/scores/run", json={})
        assert response.status_code in [
            401,
            403,
        ], "Scores endpoint allowed unauthenticated access"
